...
class SentimentAnalysis(BaseModel):
    ...
    sentiment: str | None = Field(
        default="positive", description="sentiment:positive/negative/neutral"
    )
    confidence: float = Field(
        default=0.8, description="value between 0-1", ge=0.3, le=0.99
    )
    keywords: list[str] = Field(description="list of keywords")
    recommend_level: Literal["low", "midlle", "high"] = Field(
        description="recommend level"
    )
...
result = structured_llm.invoke(f"分析下面的文本：\n{text}")
